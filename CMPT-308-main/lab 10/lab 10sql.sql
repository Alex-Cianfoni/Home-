...
begin
   open resultset for 
      select p.prereqnum, c.name
      from   Prerequisites p, Courses c
       where  Coursenum1 = p.coursenum
	   and c.num = Coursenum1;
   return resultset;
end;
$$ 
language plpgsql;

select PreReqsFor(308, 'results2');
...
begin
   open resultset for 
      select p.courseNum, c.name
      from   Prerequisites p, Courses c
       where p.prereqNum = prereq
	    and c.num = prereq;
   return resultset;
end;
$$ 
...
select IsPreReqFor(308, 'results3');
